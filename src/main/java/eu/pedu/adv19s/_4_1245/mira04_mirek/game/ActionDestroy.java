...
//\II== INSTANCE INITIALIZERS (CONSTRUCTORS) ===================================
    ...
    /***************************************************************************
     * Vytvoří novou instanci akce pro překonání překážky
     */
    ActionDestroy()
    {
        super (pROZSEKEJ,
         "Metoda ověří, zda jsme zdolali překážku.");
    }
...
//\IM== INSTANCE REMAINING NON-PRIVATE METHODS =================================
    ...
    public String execute(String... arguments)
    {
        ...
        Optional<HObjekty> oItem = currentRoom.getOItem(itemName);
        
        if(! oItem.isPresent()){
            return Texts.zNENÍ_H_OBJEKT ;}
        
        Hands hands = (Hands) MirSpaceGame.getInstance().getBag();
        Optional<HObjekty> oObject = hands.getOItem(Texts.SEKERA);
        if(!oObject.isPresent()){
            return Texts.zNENÍ_V_BATOHU;
        }
        
        if(!Texts.BARIKÁDA_BIO.equalsIgnoreCase(itemName)){
            return Texts.zTOTO_NELZE_ROZSEKAT;
        }
                
        State.setBaricadeBroken();
        return Texts.zROZSEKANO + Texts.BIO_LABORATOŘ;
    }
